...
static void history_add(const char* line) {
    ...
    if (history_count < CONSOLE_HISTORY_SIZE) history_count++;
}

static const char* history_get_from_newest(uint32_t offset) {
    if (offset >= history_count) return "";
    uint32_t idx = (history_head + CONSOLE_HISTORY_SIZE - 1 - offset) % CONSOLE_HISTORY_SIZE;
    return history[idx];
}

static void render_line(uint16_t start_pos, const char* buf, uint32_t len,
                        uint32_t* prev_len, uint32_t cursor) {
    vga_set_cursor_pos(start_pos);
    for (uint32_t i = 0; i < len;       i++) vga_putc(buf[i]);
    for (uint32_t i = len; i < *prev_len; i++) vga_putc(' ');
    *prev_len = len;
    vga_set_cursor_pos((uint16_t)(start_pos + cursor));
}

static int apply_single_completion(
    char* buffer, uint32_t* len, uint32_t* cursor,
    uint32_t line_cap, const console_completion_t* completion
) {
    uint32_t rs = completion->replace_start;
    uint32_t re = completion->replace_end;
    if (rs > *len || re > *len || rs > re) return 0;

    const char* replacement = completion->matches[0];
    uint32_t rep_len = (uint32_t)strlen(replacement);
    uint32_t new_len = *len - (re - rs) + rep_len;

    if (new_len > line_cap || new_len >= CONSOLE_HISTORY_LINE) return 0;

    char tmp[CONSOLE_HISTORY_LINE];
    uint32_t out = 0;
    for (uint32_t i = 0; i < rs;    i++) tmp[out++] = buffer[i];
    for (uint32_t i = 0; i < rep_len; i++) tmp[out++] = replacement[i];
    for (uint32_t i = re; i < *len;  i++) tmp[out++] = buffer[i];
    tmp[out] = '\0';
...
void console_read_line_opts(
    ...
    int use_history,
    const char* initial_text, const char* prompt,
    console_completion_provider_t completion, void* completion_user
) {
    if (max_len == 0) return;

    if (prompt) vga_write(prompt);
    ...
    while (1) {
        ...
        if (event.type == KEY_EVENT_ENTER) {
            ...
            vga_putc('\n');
            if (use_history) history_add(buffer);
            return;
        }
        ...
        if (keyboard_is_alt_pressed()) {
            if (event.type == KEY_EVENT_LEFT  || event.type == KEY_EVENT_RIGHT ||
                event.type == KEY_EVENT_UP    || event.type == KEY_EVENT_DOWN) {
        ...
        if (completion && event.type == KEY_EVENT_CHAR && event.ch == '\t') {
            ...
            if (info.match_count == 1) {
                ...
                for (uint32_t i = 0; i < info.match_count; i++) {
                    ...
                    if (i + 1 < info.match_count) vga_write("  ");
                }
                vga_putc('\n');
                if (prompt) vga_write(prompt);
